...
if($_GET["dni"] != "" && $_GET["nombre"] != "" && $_GET["primer_apellido"] != "" && $_GET["movil"] != "" && $_GET["fecha_nacimiento"] != "" ){
    ...
    if($dniResultado[0] == NULL){
        ...
        //http://localhost/CoachManagerPHP/CoachManager_InsertAlumno.php?nombre=Pepe&primer_apellido=Peponcio&segundo_apellido=Nepe&dni=1234&movil=1234&fecha_nacimiento=2018/05/03&genero=masculino&peso=123&altura=1234&mano_dom=derecha&pie_dom=1234&observaciones=hola&id_persona=1

        $insertAlumno = "INSERT INTO ENTRENADORES (PESO, ALTURA, MANO_DOM, PIE_DOM, OBSERVACIONES, ID_PERSONA) VALUES ({$peso}, {$altura}, '{$mano_dom}', '{$pie_dom}', '{$observaciones}', '{$id_persona[0]}')";
